...
const Content: React.FC<ContentProps> = ({ content }) => {
  ...
  return (
    <div className={`p-6 md:p-8 bg-black/10 rounded-2xl shadow-xl border border-white/10 transition-all duration-1000 ease-in-out delay-[1500ms] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
      ...
      {/* The logo and countdown are now grouped together */}
      <div className="flex flex-col items-center justify-center mb-6">
        <img
          src="/ansarullah-logo.png"
          alt="Majlis Ansarullah Logo"
          className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-yellow-300 shadow-lg mb-6"
        />
        <h1 className="text-2xl md:text-3xl font-extrabold mb-4 animate-fadeIn text-yellow-300 drop-shadow-lg">{content.title}</h1>
        <Countdown targetDate={targetDate} />
      </div>

      {/* The "Left" heading is moved below the countdown */}
